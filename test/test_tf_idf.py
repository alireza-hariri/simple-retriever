...
def method_tester(method):
    ...
    tick("fitting")
    retriever = retriever_factory(method=method)
...
def test_TFIDF_Retriever():
    ...
    tick("fitting")
    retriever = TFIDF_Retriever(analyzer="char", ngram_range=(3, 5))
    retriever.add_doc_batch(test_docs)
    retriever.fit()
    tick("finding result")
    results = retriever.find_similars("similar term!", top_k=4)
    ...
    print('test_TFIDF_Retriever passed!')
